...
void usuario() {
	cout << "AUTORES: NICOLL CARRILLO, PAULA ARCINIEGAS, MARIA RODRIGUEZ" << endl;
}
...
int main() {
	...
	// Mostrar los nombres de los autores
	usuario();
	...
	do {
		...
		case 2: {
			do {
				...
				case 5: {
					confirmarMonedas = true;  // Se marca como confirmada la opciC3n de monedas
					break;
				}
				default: {
					cout << "\nOpcion invalida. Intenta de nuevo.";//validador para que el usuario escoja una opciC3n correcta para agregar monedas
				}
				}
		...
		case 3: {
			do {
				...
				if (opcionProducto <= 4 && opcionProducto > 0) {
					if (maquina.productos[opcionProducto - 1].cantidad > 0) {
						if (cliente.saldoMonedas >= maquina.productos[opcionProducto - 1].precio) {
							...
							if (opcion == 1) {
								...
								cout << "\nSeleccion invalida!!";//validador de selecciC3n compra
								confirmarCompra = false;
							}
						} else {
							confirmarCompra = true;
					...
					confirmarCompra = true;
					cout << "\nOpcion invalida !!";//validador para opciC3n correcta
				}
		...
		case 10: {
			// Mostrar las unidades compradas por tipo de producto con donaciC3n FOPRE
			for (int i = 0; i < 4; i++) {
				if (maquina.productos[i].donacionFopre) {
					cout << "\n\nProducto: " << maquina.productos[i].nombre;
		...
		default: {
			cout << "\nOpciC3n invalida. Intenta de nuevo."; //validador para asegurar que el usuario escoja una opciC3n del menC:
			break;
		}
		}
	} while (!salirMenu);
